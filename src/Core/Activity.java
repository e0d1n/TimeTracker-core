...
public abstract class Activity implements Serializable, Printable {
     ...
     * We needed this pattern because we have to contain in 
     * a project, tasks and other projects
     * This function starts the dates to null because we need 
     * it to control when a activity is not started, to assign 
     * both
    ...
    /**
     * It's used to update the dates and duration of the activity.
     * At first we watch the dates state and we use the null state 
     * for determinate if is the first iteration in the activity, 
     * it is required to control the assignment of the start date. 
     * This along with the parameter "first" we control the 
     * initialization and update of the duration. 
     * Update the finish date with clock 
     * At last we call this function at the parent, if the activity 
     * have it
     */
